 ...
 *
 * In theory, 1 worker could handle all 5 requests. However, due to workerRequests=3,
  ...
  protected function buildPromises(PipePool $pool): array {
    return [
      $pool->dispatch('A', 'Apple 100')
        ...
        // Per workerRequests, that was the last time we could reuse the worker! New worker started.
        ->then(function($resp) use ($pool) {
